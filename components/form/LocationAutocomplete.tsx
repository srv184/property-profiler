...
export function LocationAutocomplete({ disabled, onAdd }: Props) {
  ...
  return <div className="relative mb-3 flex gap-2">
    ...
    <button type="button" onClick={() => select()} disabled={disabled || !value.trim()} className="focus-ring flex flex-shrink-0 items-center gap-1.5 rounded-lg border border-line bg-canvas-raised px-3.5 py-2.5 text-sm font-medium text-ink transition-colors hover:bg-canvas-sunken disabled:cursor-not-allowed disabled:opacity-40"><Plus size={15} /> Add</button>
    {open && items.length > 0 && <ul id={listId} role="listbox" className="absolute left-0 right-14 top-full z-10 mt-1 max-h-52 overflow-auto rounded-lg border border-line bg-canvas-raised p-1 shadow-card">{items.map((item, index) => <li key={item} id={`${listId}-${index}`} role="option" aria-selected={index === active} onMouseDown={(event) => { event.preventDefault(); select(item); }} className={`cursor-pointer rounded-md px-3 py-2 text-sm ${index === active ? "bg-canvas-sunken text-ink" : "text-ink-faint hover:bg-canvas-sunken"}`}>{item}</li>)}</ul>}
  </div>;
}
